...
# Agent Guide: FollowLang™ Compiler Suite
...
- **Elite™**: Elite Turing-completion assembly statements restricted to imperative procedurals such as (`mov`, `jmp`, `cmp`) operating on spacetime origo deltas.
- **Follower™**: Follower compiler constructs introducing complementary programming languages (such as Rust, C/C++, Delphi/FPC, Python, Prolog, etc.).
- **Ether™**: Ether IR machine-epsilon error mitigation and library linking.

---

## Follow™-Suite Architecture:

The Reactive Paradigm & Symbolic, Follow™ - QGTC/QGCT Meta-Programming Language-Suite.

## ***IMPLEMENTS THE CORE `../Follow/` STRUCTURES*** - READ BELOW.
 - BASIS.
 - NUM.
 - NUMBER.
 - INT.
...
## The Follow™-Language specifies the following set of Meta-Languages:

 - Flow™: Fractal Strings and Permutation-groups/Permutohedrons.
 - Action™: Recurring and Symbolic Moduli-spaces.
 - Maneuvers™: (Weighted) Functionals of expressions of the former two, for usage in Machine Learning.
 - United Interchange™: Arithmetic Meta-schemas of the former languages, for usage as a dataset-independent interchange language when data-mined for orthonormality.
 - Elite™: Pragmatic United Interchange Procedurals (e.g., { mov, jmp, cmp }-denoted United Interchange Meta-schemas at the bare-metal and the embedded level).
 - Follower™: Assembler of High-level Complementary Languages intersected with the Follow™-Language.
 - Ether™: Machine-Epsilon Error-Mitigation // Library Linker // Intermediate Representation Language for Follow™-Compilation.

#### A Fully-Fledged Reactive Paradigm, Symbolic Meta-Programming Language

An architecture and framework of Quantum Gravity Turing-Computing Programming Languages and Quantum Gravity Communication Theory.

# Flow Language

#### Topological Braids and Permutohedron Domain.

Syntax:

 - Utilizing Fractal Strings as "Real" and permutations of the (ringed) extended imaginary bases as "Base".
 - Written as strings of `{ ±1_n, ±i_n, ±j_n, ±k_n, ... }` or `{ R_n, B_n }`, n being ring.
 
Semantics:

 - When evaluated with another Flow language dataset invokes entire complex-fractal moduli-spaces of interest.

# Action Language

#### Moduli-spaces and Factorization Design

Syntax:

 - Grouping recurring and symbolic moduli-spaces into elementary parts - data-mined from the Flow Language or hard-coded by the programmer:
 - Written as symbolic arithmetic expression `arithmetics of integers equivalent with (mod X)`: for example `(7m + (4n * 4o)) equivalent with 0 (mod 3)`.
 
Semantics:

 - When assigned to a symbol or functional expression, asserts a framework for utilizing Flow or Actions as Symbols, Commands or Filters in Machine Learning.
   
# Maneuvers Language

#### Meta-Categorization Solution

Syntax:

 - Grouping Flow Commands and Action Symbols (or as Composite Filters) into not necessarily commutative Meta-linguistic groups/relationships.
 - Attached to a symbolic evaluation function.
 
Semantics:

 - Let's us categorize interelations and interconnections in multi-dimensional sparse-matrix fields.

# United Interchange Language

#### Meta-Schemas for Reservoir Computing

Syntax:

 - Using Flow Commands, Action Symbols and Maneuvers Functionals, the programmer can code interchange schemas between them.

Semantics:

 - Using Reservoir Computing, one can achieve orthonormality within the cohomologies of diffeomorphisms of these topologies and isometries, and - within margins of machine-epsilon error - achieve a dataset-independent workflow for multilateral cooperations across industries.

# Elite Programming

#### Natural Language Processing & Symbolic Assembler

Syntax:

 - Attaching a procedural invocation with parameters of Flow, Action and Maneuvers to United Interchange Language Meta-schemas
 - Using the Turing-complete set of `{ mov, jmp, cmp }`-denoted United interchange schemas.

Semantics:

 - Fully realizing one's own organization's potential, generating international common-grounds for inclusion, diversification and variance by assembling one's own Polymorphically & Algorithmically-Grown QGTC-Programs and QGCT-Meta-schemas.

# Follower Language

#### Main Interoperability Compilation Language

Syntax:

 - The Front-end Programming Language of the Follow-Suite, organized with United Interchange Meta-schemas and the other interoperability languages, and routines of any of the complementary languages.

Semantics:

 - Makes it possible to perform Quantum Gravity Turing-Computing like never before seen.

# Ether Intermediate Representation

#### Library Linking & Machine-Epsilon Error-Correction Language

 - Generated from the Follower Compiler.
 - Used to mitigate fuzzy-algebraic errors of small-valued machine-epsilon precision correlations in one's dataset.
 - Finally assembling the whole project of multi-linguistic parts including interoperability of complementary structures and systems in a cross-compilation friendly packaging.

# FOLLOW-CORE

USES THE CORE `../Follow/`-PROJECT WHICH SPECIFIES THE FOLLOWING TYPES:
